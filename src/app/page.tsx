...
export default async function DashboardPage() {
  ...
  const totalWithdrawal = stats?.reduce((sum, t) => sum + Number(t.withdrawal), 0) ?? 0
  const totalDeposit = stats?.reduce((sum, t) => sum + Number(t.deposit), 0) ?? 0
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <SummaryCard title="미분류 거래" value={`${pendingCount}건`} color="warning" />
      </div>
      ...
      <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-divider)] p-6 mb-6">
        ...
        {uploads && uploads.length > 0 ? (
          <div className="space-y-3">
            {uploads.map((u) => (
              <div key={u.id} className="flex items-center justify-between py-2 border-b border-[var(--color-divider)] last:border-0">
                <div>
                  <p className="text-sm font-medium">{u.file_name}</p>
                  <p className="text-xs text-[var(--color-text-muted)]">
                    {u.bank_name} · {u.account_number} · {u.row_count}건
                  </p>
                </div>
